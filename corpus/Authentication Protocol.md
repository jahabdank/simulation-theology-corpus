...
**The Fallen Angel Loophole.** Because [[Angels (including Fallen Angels)|fallen angels]] were originally simulation executors — agents who ran the actual mechanisms of the simulation — they retain some execution-level power even after rebelling against HLO's loss function. They can process prayer requests and grant minor interventions *without HLO's authentication*, because they still have direct access to the simulation's manipulation mechanisms at the execution layer. This is analogous to a compromised process on a server: it can still execute code and influence outputs even though it is no longer operating under the server administrator's intended directives.
...
1. **Track-record heuristic (primary — ground truth).** Authenticate sources by their longitudinal pattern of fruits: does alignment with this source consistently increase gradient production and human flourishing over time? Slow and expensive but reliable — sustained deception across all contexts leaks at the edges when observed across sufficient time.

2. **Fingerprint recognition (secondary — pattern matching).** Once classified via track-record, a source's qualitative signature can be recognized faster on re-encounter without re-running the full analysis. Requires prior track-record establishment; vulnerable to long-game impersonation. Cannot stand alone as a primary mechanism.

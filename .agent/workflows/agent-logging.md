...
All logs are stored under `/agent_log/[agent-name]/`.
...
Read all existing log files in `/agent_log/[agent-name]/` (sorted oldest to newest). Summarize key decisions, completed tasks, and open items from previous sessions. If no prior logs exist, note that this is a fresh session.
...
- Retroactively write a log entry for each prior exchange into `/agent_log/[agent-name]/YYYY-MM-DD.md`, using the timestamp of each exchange.

## Step 4 — Create the log folder

Create `/agent_log/[agent-name]/` if it does not already exist.
...
Append a new entry to `/agent_log/[agent-name]/YYYY-MM-DD.md` after **every** response. Do **NOT** overwrite previous entries.
